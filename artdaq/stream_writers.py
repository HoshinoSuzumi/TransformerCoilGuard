...
from artdaq._task_modules.write_functions import (
    _write_analog_f_64, _write_analog_scalar_f_64, _write_binary_i_16, _write_binary_u_16,
...
class ChannelWriterBase(object):
    """
    Defines base class for all ArtDAQ stream writers.
    """
    ...
    def __init__(self, task_out_stream, auto_start=AUTO_START_UNSET):
        ...
        Args:
            task_out_stream: Specifies the output stream associated with
                an ArtDAQ task which to write samples.
                If you do not specify a value for this parameter,
                ArtDAQ determines its value based on the type of write
    ...
    def auto_start(self):
        ...
        bool: Specifies if the write method automatically starts the
            task if you did not explicitly start it with the Start
            Task method.

            If you do not specify a value for this parameter, ArtDAQ
    ...
    def _raise_error_if_invalid_write_dimensions(
        ...
        if num_dimensions_expected != num_dimensions_in_data:
            raise DaqError(
                ...
                Errors.UNKNOWN.value, task_name=self._task.name)


class AnalogSingleChannelWriter(ChannelWriterBase):
    """
    Writes samples to an analog output channel in an ArtDAQ task.
    """
...
class AnalogMultiChannelWriter(ChannelWriterBase):
    """
    Writes samples to one or more analog output channels in an ArtDAQ
    task.
    """
...
class CounterWriter(ChannelWriterBase):
    """
    Writes samples to a counter output channel in an ArtDAQ task.
    """
    ...
    def write_one_sample_pulse_frequency(
        ...
        Args:
            ...
            duty_cycle (float): Specifies the width of the pulse divided
                by the pulse period. ArtDAQ uses this ratio combined
...
class DigitalSingleChannelWriter(ChannelWriterBase):
    """
    Writes samples to a single digital output channel in an ArtDAQ
    task.
    """
...
class DigitalMultiChannelWriter(ChannelWriterBase):
    """
    Writes samples to one or more digital output channels in an ArtDAQ
    task.
    """
